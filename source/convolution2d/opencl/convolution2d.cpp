...
// Fused: kernel weights loaded into local memory cooperatively
__kernel void convolution2d_fused(__global const float* input,
                                  __global const float* kernel,
                                  __global float* output,
                                  int input_rows, int input_cols,
                                  int kernel_rows, int kernel_cols,
                                  int out_rows, int out_cols,
                                  __local float* s_kernel) {
    int lid = get_local_id(1) * get_local_size(0) + get_local_id(0);
    int block_threads = get_local_size(0) * get_local_size(1);
    int kernel_elems = kernel_rows * kernel_cols;

    for (int i = lid; i < kernel_elems; i += block_threads)
        s_kernel[i] = kernel[i];
    ...
    if (out_row < out_rows && out_col < out_cols) {
        ...
        for (int kr = 0; kr < kernel_rows; kr++)
            for (int kc = 0; kc < kernel_cols; kc++)
                acc += input[(out_row + kr) * input_cols + (out_col + kc)]
...
void convolution2d_gpu2(const float* h_input, const float* h_kernel, float* h_output,
    ...
    cl_kernel kernel_obj = clCreateKernel(prog, "convolution2d_fused", &err);
    CL_CHECK(err);
    CL_CHECK(clSetKernelArg(kernel_obj, 0, sizeof(cl_mem), &d_input));
    CL_CHECK(clSetKernelArg(kernel_obj, 1, sizeof(cl_mem), &d_kernel));
    CL_CHECK(clSetKernelArg(kernel_obj, 2, sizeof(cl_mem), &d_output));
    ...
    CL_CHECK(clSetKernelArg(kernel_obj, 7, sizeof(int), &out_rows));
    CL_CHECK(clSetKernelArg(kernel_obj, 8, sizeof(int), &out_cols));
    CL_CHECK(clSetKernelArg(kernel_obj, 9, sizeof(float) * kernel_rows * kernel_cols, nullptr));
